...
- `outline`：已经划定问题边界，但正文尚未系统展开。
- `draft`：已经形成可读正文，但仍需要进一步核验或补充来源。
- `reviewed`：结构、术语、主要事实、关键来源和内部链接均已检查。
- `needs-update`：内容可能因新研究、规范或行业实践变化而过时。

状态描述的是证据与校验程度，不是文字是否流畅。第一版主要由 `outline` 和 `draft` 构成。
